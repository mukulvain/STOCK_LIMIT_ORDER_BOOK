...
from .models import Order, Stoploss_Order, Trade, User
from .utils import match_order  # Assuming match_order is in utils.py
from .utils import \
    broadcast_orderbook_update  # Assuming broadcast_orderbook_update is in utils.py
...
def fetch_best_ask():
    ...
    return (
        Order.objects.filter(order_type="SELL", is_matched=False)
        .order_by("price")
...
def fetch_best_bid():
    ...
    return (
        Order.objects.filter(order_type="BUY", is_matched=False)
        .order_by("-price")
        .values("price", "quantity")
        .first()
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if stoploss_order == "NO" or stoploss_order == None:
                ...
                if (
                    disclosed < 0.1 * quantity
                ):  # disclosed_quantity should not be > 10% of quantity
                    messages.error(
                        request,
                        "Disclosed Quantity cannot be less than 10% greater than Quantity.",
                    )
                ...
                if (
                    ...
                    broadcast_orderbook_update()
                    messages.success(
                        request, "Your Stoploss order has been placed successfully!"
                    )
                    return redirect("/home")
...
def orderbook(request):
    ...
    buy_orders = Order.objects.filter(is_matched=False, order_type="BUY").order_by(
        "-price"
    )
    # Retrieve unmatched sell orders (sorted by price in ascending order)
    sell_orders = Order.objects.filter(is_matched=False, order_type="SELL").order_by(
        "price"
    )
...
def update_prev_order(request):
    if request.method == "POST":
        try:
            ...
            print(
                f"Received order update: Order ID = {order_id}, Quantity = {new_quantity}, Disclosed Quantity = {new_disclosed}, Price = {new_price}"
            )
            ...
            if order.is_matched == True:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "Order has already been placed. No modifications allowed.",
                    }
                )
            if new_disclosed < new_quantity * 0.1:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "Disclosed value must be greater then 10% of quantity.",
                    }
                )
            if new_disclosed > new_quantity:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "Cannot disclose more than the quantity.",
                    }
                )
            if new_price <= 0:
                return JsonResponse(
...
def get_recent_trades(request):
    if request.method == "GET":
        recent_trades = (
            ...
            .values("buyer", "seller", "price", "quantity", "timestamp")
...
def cancel_order(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "User authentication failed"}, status=401
            )
        except Order.DoesNotExist:
            return JsonResponse(
                {"success": False, "message": "Order not found or already matched"},
                status=404,
            )
        ...
        try:
            ...
            with transaction.atomic():
                order = Stoploss_Order.objects.get(
                    id=order_id, user=user, is_matched=False
                )
                order.delete()

            return JsonResponse(
                {"success": True, "message": "Stoploss order cancelled successfully"}
            )
...
from .models import Order, Stoploss_Order
...
def execute_order():
    ...
    stop_loss_buy_orders = Stoploss_Order.objects.filter(order_type="BUY").order_by(
        "target_price"
    )
    stop_loss_sell_orders = Stoploss_Order.objects.filter(order_type="SELL").order_by(
        "-target_price"
    )
