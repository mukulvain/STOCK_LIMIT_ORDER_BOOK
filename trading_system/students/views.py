        ...
        if form.is_valid():
            ...
            for row in csv_reader:
                if len(row) >= 2:  # Ensure at least two columns exist
                    ...
                    password = (
                        row[2] if len(row) > 2 else "defaultpassword"
                    )  # Set password if given, else default
            ...
            messages.success(
                request,
                f"Users created successfully! and email sent to {success_count} users. and error in {error_count} users",
            )
...
def bulk_user_delete(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if deleted_count > 0:
                messages.success(
                    request, f"{deleted_count} users deleted successfully."
                )
            if not_found_users:
                messages.warning(
                    request, f"Users not found: {', '.join(not_found_users)}"
                )
...
def change_password(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Your password was not updated! Please correct the error below.",
            )
    else:
        form = PasswordChangeForm(request.user)
    return render(request, "trading/reset_password.html", {"form": form})
